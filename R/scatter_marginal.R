...
#'
#' This function will take in two vectors of equal length
#'
...
scatter_marginal <- function(x1, x2) {
  if (length(x1)==length(x2)) {
    data <- data.frame(x1,x2)
    scatter_plot <- ggplot2::ggplot(data, ggplot2::aes(x = x1, y = x2)) +
      ggplot2::geom_point(size=3) +
      ggplot2::theme_minimal() +
      ggplot2::labs(x="Age of Car (years)", y="Price of Car (thousands of dollars)")
    ggExtra::ggMarginal(scatter_plot, type="histogram", margins="both")
